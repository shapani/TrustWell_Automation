Feature: 02_Calories Calculation functionality
  ...
  Scenario Outline: 01_Calories: The Calories displayed on the dashboard should match with the calories calculated based in the formula.
   ...
   Scenario Outline: 02_Calories: Verify the user can edit the nutrient and the new value gets reflected on the dashboard.
    When I enter username as "<userName>"
    And I enter password as "<passWord>"
    And I click on Continue button
    And I click on Add Nutrient button on Home page
   ...
   Examples:
      ...
      | testUser | Test1234 | 0.0           | -2.2        | 1.07             | -1                |  12           |   5.70     | 0.13            |

@Calories @deleteCalories @TC_CALC_03 @Regression
   Scenario Outline: 03_Calories: Verify the user can delete the exiting nutrient and the nutrient gets deleted from the Dashboard.
    When I enter username as "<userName>"
    And I enter password as "<passWord>"
    And I click on Continue button
    And I click on Add Nutrient button on Home page
  ...
  @Calories @noUpdates @TC_CALC_04 @Regression
   Scenario Outline: 04_Calories: Verify the nutrient should not be added/edited after clicking on Cancel/Close button on Add Nutrient Dialog box.
    When I enter username as "<userName>"
    And I enter password as "<passWord>"
    And I click on Continue button
    And I click on Add Nutrient button on Home page
  ...
  @Calories @noUpdates @TC_CALC_05 @Regression
   Scenario Outline: 05_Calories: Verify the new calories value as per formula on the dashboard after updating the editing the nutrient values.
    When I enter username as "<userName>"
    And I enter password as "<passWord>"
    And I click on Continue button
    And I click on Add Nutrient button on Home page
   ...
   Examples:
      | userName | passWord | orgCarbsValue | orgFatValue | orgAlcoholValue | OrgSugarAlcoholVal | newCarbsValue |
      | testUser | Test1234 | 20.0          | 4.5         | 1.07             | 12.5              |  -4           |